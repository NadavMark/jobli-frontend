...
export default function PostJobWizardStep({ next }) {
    ...
    return (
        <View accessible={true} style={styles.wrapper}>
            ...
            <ReactChipsInput
                label=" "
                accessibilityLabel="הוסף תפקיד"
                initialChips={list}
                onChangeChips={(updatedList) => updateList(updatedList)}
                chipStyle={styles.selectedChipStyle}
                inputStyle={styles.inputStyle}
                labelStyle={{ color: Theme.c3 }}
                labelOnBlur={{ color: Theme.c2 }} />
            <View style={{ flexDirection: 'row', alignContent: 'stretch' }}>
                {optionalValues.filter(item => !list.includes(item)).map((item, index) => (
                    <Chips
                        key={index}
                        value={item}
                        chipBtnText="+"
